...
import { Box, Button, useDisclosure, Table, Thead, Tbody, Tr, Th, Td, TableContainer, Modal, ModalOverlay, ModalContent, ModalHeader, Tooltip, Heading, ModalFooter, ModalBody, ModalCloseButton, Checkbox, Input, Text, Select, useToast, Menu, MenuButton, MenuList, MenuItem, Popover, PopoverTrigger, PopoverContent, PopoverBody, PopoverArrow, PopoverCloseButton, Spinner } from "@chakra-ui/react";
import { DragDropContext, Droppable, Draggable } from "react-beautiful-dnd";
import debounce from "lodash/debounce";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faChartLine, faGear } from "@fortawesome/free-solid-svg-icons";
import { saveAs } from "file-saver";
import { faFileExcel, faArrowDownShortWide, faArrowUpWideShort, faArrowRightArrowLeft } from "@fortawesome/free-solid-svg-icons";
...
const CustomTable = ({ setPage, newArray, alignment, filters, setFilters }) => {
  ...
  const handleApplyChanges = () => {
    ...
    onClose();
    localStorage.setItem("selectedColumnSalesCustomer", JSON.stringify(updatedSelectedColumns));
    toast({
  ...
  useEffect(() => {
    const savedColumns = localStorage.getItem("selectedColumnSalesCustomer");
    if (savedColumns) {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Advanced filters
    else if (advancedFiltercustomer?.content && advancedFiltercustomer?.content.length > 0) {
      filteredData = advancedFiltercustomer.content;
    }
    return filteredData;
  }, [
    newArray,
  ...
  const formatHeader = (column) => {
    if (columnDatacustomer && columnDatacustomer.content) {
      const apiContent = columnDatacustomer.content[0];
                        ...
                        {selectedColumns.map((column, colIndex) => (
                          ...
                          >
                            <Text
                              fontSize="1.4rem"
                              justifyItems="center"
                              fontWeight={column === "customer.trade_name" ? "600" : "400"}
                              whiteSpace="nowrap"
                              color="#4e4e4e"
                              width={
                                column === "description"
                              ...
                              title={column === "customer.trade_name" ? item[column] : undefined}
