...
export const createBodyWise = (data, startDate, endDate, selectData, type = "", selectedWise = '', reportType = '') => {
    if (data === 'month') {
        if (reportType === "sales") {
            const xaxis = selectedWise === "sales-kam-wise" ? "kam.kamName" : "items.itemName";
            const groupBy = selectedWise === "sales-kam-wise" ? ["kam.kamCode"] : ["items.itemName"];
            if (type === "bar") {
                return {
                    "xaxis": xaxis,
                    "yaxis": [
            ...
            } else if (type === "bump" || type === "areaBump" || type === "line") {
                return {
