...
import { Box, Button, useDisclosure, Table, Thead, Tbody, Tr, Th, Td, TableContainer, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, Heading, Checkbox, Input, Text, Select, useToast, Menu, MenuButton, MenuList, MenuItem, Popover, PopoverTrigger, PopoverContent, PopoverBody, PopoverArrow, PopoverCloseButton, Tooltip, Spinner } from "@chakra-ui/react";
...
import { useGetSelectedColumnsSoQuery } from "../slice/salesSoWiseApi";
import { useGetGlobalsearchQuery } from "../slice/salesSoWiseApi";
...
import { useGetexportdataSalesSoQuery } from "../slice/salesSoWiseApi"
...
const CustomTable = ({ setPage, newArray, alignment, filters, setFilters }) => {
  ...
  //API Calling sorting
  const { data: SoWise } = useSoWiseSalesQuery({
    filters: {
      ...filters,
      sortBy: sortColumn,
      sortDir: sortOrder,
  ...
  // ...Export API CALL...
  const { data: exportData } = useGetexportdataSalesSoQuery(localFiltersdate || {}, { skip: !triggerApiCall });
  ...
  const handleApplyChanges = () => {
    ...
    onClose();
    localStorage.setItem("selectedColumnSalesSo", JSON.stringify(updatedSelectedColumns));
    toast({
  ...
  useEffect(() => {
    const savedColumns = localStorage.getItem("selectedColumnSalesSo");
    if (savedColumns) {
  ...
  //toast message for no data filter
  useEffect(() => {
    if (SoWiseFilter?.content && SoWiseFilter.content.length === 0) {
      toast({
  ...
  //toast message for no data search
  useEffect(() => {
    if (searchData?.content && searchData.content.length === 0) {
      toast({
        title: " No Results Found ",
